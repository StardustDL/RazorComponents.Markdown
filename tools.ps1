if ($args.Count -gt 0) {
    switch ($args[0]) {
        "npmup?" {
            Set-Location src/RazorComponents.Markdown && ncu && Set-Location ../.. || exit 1
        }
        "npmup" {
            Set-Location src/RazorComponents.Markdown && ncu -u && npm install && Set-Location ../.. || exit 1
        }
        "restore" {
            Write-Output "Restore npm..."
            Set-Location src/RazorComponents.Markdown && npm ci && gulp && Set-Location ../.. || exit 1
